...
def create_attn_mask(
    ...
    if is_causal:
        ...
        causal_mask_2d = torch.tril(torch.ones(
            (Tq, Tk), dtype=torch.bool, device=device))  # Shape [Tq, Tk]
...
class EncoderInferenceState:
    ...
    def new(cls, config: DiaConfig, cond_src: torch.Tensor) -> "EncoderInferenceState":
        ...
        positions = (
            torch.arange(config.data.text_length, dtype=torch.float32,
                         device=device).unsqueeze(0).expand(2, -1)
        )
        padding_mask = (cond_src != config.data.text_pad_value).to(
            device).expand(2, -1)
...
class KVCache:
    def __init__(
        ...
        self.k = torch.zeros((2, num_heads, max_len, head_dim),
                             dtype=dtype, device=device) if k is None else k
        self.v = torch.zeros((2, num_heads, max_len, head_dim),
                             dtype=dtype, device=device) if v is None else v
...
class DecoderInferenceState:
    ...
    def prepare_step(self, step_from: int, step_to: int | None = None) -> None:
        if step_to is None:
            step_to = step_from + 1
        self.dec_positions = (
            torch.arange(step_from, step_to, dtype=torch.float32,
                         device=self.device).unsqueeze(0).expand(2, -1)
        )
